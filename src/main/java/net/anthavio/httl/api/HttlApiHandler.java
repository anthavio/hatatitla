...
public class HttlApiHandler<T> implements InvocationHandler {
	...
	public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
		...
		if (args != null) {
			for (int i = 0; i < args.length; ++i) {
				...
				if (metaVar.setter != null) {
					...
					*/
					case REQ_MARSHALLER:
						requestMarshaller = (RequestMarshaller) arg;
						break;
					case RES_EXTRACTOR:
						responseExtractor = (ResponseExtractor<?>) arg;
						break;
					default:
		...
		//custom marshaller to build request body...
		String marshalledBody = null;
		if (requestMarshaller != null && body != null) {
			marshalledBody = requestMarshaller.marshall(body);
		}

		if (metaMethod.httpMethod.getMethod().isBodyAllowed()) {
			String contentType = builder.getHeaders().getFirst(HttlConstants.Content_Type);
			if (marshalledBody != null) {
				builder.body(marshalledBody, contentType);
			} else if (body != null) {
				builder.body(body, contentType);
			}
		}
		...
		if (metaMethod.builder == null) {
			return complete(builder, metaMethod, builderInterceptor, executionInterceptor, responseExtractor);
		} else {
			...
			HttlCallBuilderHandler handler = new HttlCallBuilderHandler(this, metaMethod, builder, builderInterceptor,
					executionInterceptor, responseExtractor);
	...
	protected Object complete(SenderBodyRequestBuilder builder, ApiMethodMeta metaMethod,
			...
			ResponseExtractor<?> responseExtractor) throws IOException {
